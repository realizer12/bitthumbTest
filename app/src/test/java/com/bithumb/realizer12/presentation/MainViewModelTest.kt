...
import com.bithumb.realizer12.presentation.rule.MainCoroutineRule
...
class MainViewModelTest {
    ...
    class FakeRepository(private val photoDataModel: PhotoDataModel):PhotoRepository{
        override suspend fun getPhoto(pageStart: Int): Flow<Result<List<PhotoDataModel>>> {
            return flowOf(Result.success(listOf(photoDataModel)))
        }
    ...
    @Test
    fun testGetNewPhotoOnSuccess2():Unit = runTest{

        photoRepository = FakeRepository(data)
        mainViewModel = MainViewModel(photoRepository)
